<template>
  <div class="wrapper">
    <div class="row">
      <div class="col-md-4">
        <img class="profil" src="/unnamed.jpg" width="250" /> <br />
        <br />
        <h4 class="nom">Privat Kouadio</h4>
        <p class="prefession">Developpeur web et application mobile</p>
        <br />
        <button class="btn-contacter" type="button">Contacter</button> <br />
        <br />
        <p class="bio">
          🚀 Passionné par le développement web, j'ai décidé de me plonger corps
          et âme dans NUXT.js pour atteindre une expertise hors norme dans ce
          framework modern.
        </p>
        <br />

        <div class="tech">
          <h5>Mes technologie</h5> <br>
          <div class="row">
            <div class="col">
              <img src="/logo/nuxt.png" alt="" />
            </div>
            <div class="col">
              <svg
                fill="none"
                height="91"
                viewBox="0 0 73 91"
                width="73"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M22.5752 87.933C26.3634 89.4568 30.4722 90.3615 34.7873 90.5132C40.6261 90.717 46.1816 89.5089 51.1455 87.2147C45.1923 84.8757 39.8009 81.4554 35.1974 77.2024C32.2171 81.9798 27.805 85.7506 22.5752 87.933Z"
                  fill="#FF9100"
                ></path>
                <path
                  d="M35.1996 77.2049C24.6952 67.4909 18.3219 53.4295 18.8613 38.0059C18.8787 37.5063 18.906 37.0042 18.9359 36.5046C17.0542 36.0174 15.0905 35.7216 13.0697 35.6495C10.1764 35.5476 7.37501 35.908 4.73026 36.6512C1.92643 41.5629 0.233686 47.1979 0.0224039 53.2356C-0.521958 68.8158 8.90619 82.4273 22.5749 87.9331C27.8047 85.7532 32.2168 81.9849 35.1996 77.2049Z"
                  fill="#FFC400"
                ></path>
                <path
                  d="M35.1998 77.2047C37.6433 73.2973 39.1222 68.7137 39.2962 63.7772C39.7486 50.792 31.019 39.6214 18.9361 36.5044C18.9063 37.004 18.8789 37.5061 18.8615 38.0057C18.3246 53.4268 24.6954 67.4883 35.1998 77.2047Z"
                  fill="#FF9100"
                ></path>
                <path
                  d="M37.9435 0C31.0632 5.51321 25.6271 12.7813 22.341 21.1555C20.4594 25.9529 19.2762 31.1032 18.9307 36.5045C31.0135 39.6216 39.7432 50.7922 39.2883 63.7798C39.1168 68.7163 37.6304 73.2949 35.1919 77.2074C39.7929 81.4653 45.1868 84.8806 51.14 87.2196C63.0911 81.6965 71.5697 69.81 72.0594 55.7511C72.3775 46.6411 68.8777 38.5229 63.9337 31.6699C58.7113 24.4242 37.9435 0 37.9435 0Z"
                  fill="#DD2C00"
                ></path>
              </svg>
            </div>
            <div class="col">
              <img src="/logo/vercel.png" alt="" />
            </div>
            <div class="col">
              <img src="/logo/boostrap.png" alt="" />
            </div>
          </div>
        </div>
      </div>

      <div class="col-md-8">
        <p>right</p>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  name: "IndexPage",
};
</script>

<style scoped>
* {
  margin: 0%;
  padding: 0%;
  box-sizing: border-box;
}

body {
  min-height: 100vh;
}

.col-md-4 {
  border: 1px dashed rgb(27, 85, 143);
  min-height: 100vh;
  padding-left: 160px;
  padding-top: 50px;
  padding-right: 10px;
}

.col-md-4 .btn-contacter {
  width: 100%;
  height: 35px;
  border: 1px solid rgba(128, 128, 128, 0.486);
  border-radius: 7px;
}

.profil {
  border-radius: 100%;
}
.col-md-8 {
  border: 1px dashed rgb(95, 36, 114);
  padding-right: 160px;
  padding-top: 50px;
  min-height: 100vh;
}


.tech img {
  width: 60px;
  border-radius: 100%;
}

.tech svg{
  width: 50px;
  height: 60px;
}
</style>
